...
void test_basic_do_finish_create(void) {
    unsigned long t1, t2;
    unsigned long num = 0;
    int i;
...
void test_basic_nonblocking_async_create(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    int i;
...
void test_basic_N_nonblocking_asyncs_create(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    int i, j;
...
void test_basic_1_blocking_asyncs_create(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    int i;
    ...
    printf("Average time per 1 blocking asyncs inside one do{ }finish(): %lu cycles (%s)\n", 
          (t2 - t1)/NUM_SWITCH_MEASUREMENTS, 
          num == NUM_SWITCH_MEASUREMENTS ? "Passed" : "Failed");
    return;   
}

void test_basic_N_blocking_asyncs_create(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    int i, j;
...
void test_basic_N_blocking_asyncs_create_pts(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    int i, j;
...
void test_basic_N_blocking_id_asyncs(void){
    unsigned long t1, t2;
    unsigned long num = 0;
...
void test_basic_N_blocking_id_asyncs_pts(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    unsigned int ids[NUM_INNER_ASYNCS];
    int i, j;
...
void test_basic_N_blocking_id_asyncs_and_N_yields_back(void){
    unsigned long t1, t2;
    unsigned long num = 0;
    unsigned int ids[NUM_INNER_ASYNCS];
    int i, j;
         ...
         DO_FINISH({
             for (j = 0; j < NUM_INNER_ASYNCS/2; j++) {
                 ASYNC({
                     ...
                     assert(ids[j]);
		     //printf("Ready to yield, saving myself into:%d\n", ids[j]);
                     THCYieldAndSave(ids[j]);
		     //printf("Yielded back:%d\n", ids[j]);
...
void test_basic_N_blocking_id_asyncs_and_N_yields_back_extrnl_ids(void){
    unsigned long t1, t2;
    unsigned long num = 0;
