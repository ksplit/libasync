...
#ifdef LCD_DOMAINS
//  printk("1: %s, current %p | c->ptstate %p | PTS() %p\n",
...
EXPORT_SYMBOL(THCYieldToAweNoDispatch_TopLevel);
...
static void thc_set_pts_0(PTState_t *st) {
  current->ptstate = st;
#ifdef LCD_DOMAINS
//  printk("%s, current %p | c->ptstate %p | PTS() %p\n",
//		__func__, current, current->ptstate, PTS());
#endif
}
#endif
...
EXPORT_SYMBOL(_thc_schedulecont);
/*
